...
oraclelinux-image: $(ORA_RPM)
	...
	docker build -f oraclelinux/Dockerfile $(BUILD_ARGS) $(LEGACY_TABLESPACE) -t "$(DOCKER_IMAGE):$(VERSION)-oraclelinux_legacy-tablespace" .
...
ubuntu-image: $(ORA_RPM)
	...
	docker build $(BUILD_ARGS) $(LEGACY_TABLESPACE) -t "$(DOCKER_IMAGE):$(VERSION)_legacy-tablespace" .
...
alpine-image: $(ORA_RPM) sgerrand.rsa.pub glibc-2.29-r0.apk
	...
	docker build -f alpine/Dockerfile $(BUILD_ARGS) $(LEGACY_TABLESPACE) -t "$(DOCKER_IMAGE):$(VERSION)-alpine_legacy-tablespace" .
